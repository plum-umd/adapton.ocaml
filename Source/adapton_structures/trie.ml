...
module SMakePlace(E :
  ...
  let eager_internal_struct_add : Key.t -> elt -> BS.t -> int -> t -> t =
    let loop = Art.mk_mfn
        (Key.gensym "Trie.MakePlace#internal_struct_add")
        (module AdaptonTypes.Tuple5(Key)(E)(BS)(AdaptonTypes.Int)(Data))
        (fun loop (nm, e, bs, h, t) -> match t with
           | Empty _ -> Atom (bs, S.singleton e)
           | Node (bs, t, t') ->
             ...
             if h mod 2 = 0
             then
               let art = loop.Art.mfn_art (nm'', e, (BS.prepend 0 bs), (h lsr 1), t) in
               ignore (Art.force art) ;
               Node (bs, Art art, t')
             else
               let art = loop.Art.mfn_art (nm'', e, (BS.prepend 1 bs), (h lsr 1), t') in
               ignore (Art.force art) ;
               Node (bs, t, Art art)
           | Atom (bs, es) when E.place (S.choose es) = E.place e -> (* <-- assumes no collisions *)
             Atom (bs, S.add e (S.remove e es))
           | Art a -> loop.Art.mfn_data (nm, e, bs, h, (Art.force a))
           | Root _ -> assert false (* <-- Don't pass Roots to loop *)
           | (Atom _) as t  ->
             let nm', nm'' = Key.fork nm in
             Art.force (loop.Art.mfn_art (nm'', e, bs, h, (split_atomic t))))
    in
    (fun nm e bs h t ->
       let nm', nm'' = Key.fork nm in
       Art.force (loop.Art.mfn_art (nm'', e, bs, h, t)))

  let eager_struct_add : Key.t -> t -> elt -> t =
    (fun nm t e -> eager_internal_struct_add nm e (0, 0) (E.place e) t)

  let eager_struct_deep_add : Key.t -> int -> t -> elt -> t =
    let loop = Art.mk_mfn
        (Key.gensym "Trie.MakePlace#internal_struct_add")
        (module AdaptonTypes.Tuple7(Key)(AdaptonTypes.Int)(E)(BS)(AdaptonTypes.Int)(AdaptonTypes.Int)(Data))
        (fun loop (nm, min, e, bs, h, m, t) -> match t with
           | Empty _ when m = min -> Atom (bs, S.singleton e)
           | t'      when m = min -> eager_internal_struct_add nm e bs h t'
           | Empty _ ->
             let nm', nm'' = Key.fork nm in
             if h mod 2 = 0
             then
               let zerobs = BS.prepend 0 bs in
               let art = loop.Art.mfn_art (nm'', min, e, zerobs, (h lsr 1), (m+1), (Empty zerobs)) in
               ignore (Art.force art) ;
               Node (bs, Art art, Empty (BS.prepend 1 bs))
             else
               let onebs = BS.prepend 1 bs in
               let art = loop.Art.mfn_art (nm'', min, e, onebs, (h lsr 1), (m+1), (Empty onebs)) in
               ignore (Art.force art) ;
           ...
           | Node (bs, t, t') ->
             ...
             if h mod 2 = 0
             then
               let art = loop.Art.mfn_art (nm'', min, e, (BS.prepend 0 bs), (h lsr 1), (m+1), t) in
               ignore (Art.force art) ;
               Node (bs, Art art, t')
             else
               let art = loop.Art.mfn_art (nm'', min, e, (BS.prepend 1 bs), (h lsr 1), (m+1), t') in
               ignore (Art.force art) ;
               Node (bs, t, Art art)
           | Art a -> (* <-- handling in a single case maintains the invariant
             let nm', nm'' = Key.fork nm in (* that Names always surround Arts  *)
             Name (nm', Art (loop.Art.mfn_nart nm'' (nm'', min, e, bs, h, m, (Art.force a))))*)
             loop.Art.mfn_data (nm, min, e, bs, h, m, (Art.force a))
           (* Why does this make sense? Deep add is ensuring that every recursive call, other
    ...
    (fun nm md t e ->
       let nm', nm'' = Key.fork nm in
       Art.force (loop.Art.mfn_art (nm'', md, e, (0, 0), (E.place e), 1, t)))

  let rec nadd nm t e = match t with
    | Root (md, t) -> Root (md, eager_struct_deep_add nm md t e)
    | Art    a     -> nadd nm (Art.force a) e
    | Name (nm, t) ->
      let nm', nm'' = Key.fork nm in
      Name (nm', nadd nm'' t e)
